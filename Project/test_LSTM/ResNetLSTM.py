...
class net:
    def __init__(self, hidden=2000, lr=0.001):
        features = 18432
        layers = 2
        output = 1
        self.frames = 3
    ...
    def picRead(self, dirpath=None):
        ...
        label = []
        sampleNum = 0
        for pat in os.listdir(dirpath):
            spath = dirpath+os.sep+pat
            frames = []
    ...
    def normalize(self):
        ...
        frames = self.frames
        ndata = torch.zeros(sampleNum, frames, 3, 448, 448)
    ...
    def extractFeature(self):
        ...
        ndata = torch.zeros(sampleNum, frames, 18432)
        with torch.no_grad():
    ...
    def train(self, epochNum=40, batchNum=8, finalLoss=1e-5):
        '''
        对数据进行训练
        ...
        for epoch in range(epochNum):
            ...
            with torch.no_grad():
                ...
                if(test_loss < min_loss) or (test_loss < finalLoss):
                    min_loss = test_loss
    ...
    def eval(self, samplePath):
        ...
        with torch.no_grad():
            print('开始加载')
            sample = torch.zeros(3, 3, 448, 448)
            for j in range(1, self.frames+1):
                imgPath = samplePath+os.sep+'200-V{}-OCT.jpg'.format(j)
...
if __name__ == '__main__':
    n = net()
    n.loadData("F:\\Dataset\\AMD_TimeSeries_CL\\1_to_16_joint_resize\\")
    n.train()
    n.load('best_model.pth')
    n.eval("F:\\Dataset\\AMD_TimeSeries_CL\\test\\200\\")
'''
附:
