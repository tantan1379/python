...
des_path = r"F:\Dataset\AMD\AMD_Origin_2d_v1"
...
for i in range(len(real_index_list)):
    ...
    for one_img in os.listdir(os.path.join(root_path,"01-A-0"+str(real_index_list[i])+"-V1-OCT")):
        ...
        img_array = cv_imread(one_img_path)
        cropped_img_array = img_array[0:430,500:988]
        # img = cv2.imshow(cropped_img_array)
        # plt.axis("off")
        cv_imwrite(os.path.join(des_path,real_index_list[i]+"-V1",one_img),cropped_img_array)