...
class Dynamic_SPP_Caps_UNet(nn.Module):
    def __init__(self,in_channels=1,num_classes=4,ccm=True,
        ...
        super(Dynamic_SPP_Caps_UNet,self).__init__()
        ...
        self.bn_c5_caps = nn.Sequential(
            nn.Conv2d(self.expan[-1],self.expan[-1],kernel_size=1,bias=False),
            nn.BatchNorm2d(self.expan[-1])
        )
    ...
    def forward(self, x):
        ...
        c5_out = self.bn_c5_caps(c5_caps_reshape)


        d4=self.relu(self.decoder5(c5_out)+c4_sp)  #256
...
if __name__ == '__main__':
    ...
    model = Dynamic_SPP_Caps_UNet(num_classes=2)
    import numpy as np
