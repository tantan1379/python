...
class UNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        maxpool1 = self.maxpool(conv1)       # 32*256*256
        
        conv2 = self.conv2(maxpool1)         # 64*256*256
        maxpool2 = self.maxpool(conv2)       # 64*128*128

        conv3 = self.conv3(maxpool2)         # 128*128*128
        maxpool3 = self.maxpool(conv3)       # 128*64*64

        conv4 = self.conv4(maxpool3)         # 256*64*64
        maxpool4 = self.maxpool(conv4)       # 256*32*32
        ...
        up1 = self.up_concat1(up2,conv1)     # 32*512*512
...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm, n=2, ks=3, stride=1, padding=1):
        ...
        p = padding
        if is_batchnorm:
            self.conv1 = nn.Sequential(nn.Conv2d(in_size, out_size, ks, s, p),
                                     nn.BatchNorm2d(out_size),
                                     nn.ReLU(inplace=True))
            self.conv2 = nn.Sequential(nn.Conv2d(out_size, out_size, ks, s, p),
                                     nn.BatchNorm2d(out_size),
                                     nn.ReLU(inplace=True))
