...
#------------------------UNet Layer---------------------
# n为每个unetconv2的conv个数,ks为每个卷积核的尺寸
# is_batchnorm表示每个卷积序列是否有batchnorm2d
...
class unetUp_cat(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, n_concat=2):
        super(unetUp_cat, self).__init__()
        self.conv = unetConv2(in_size+(n_concat-2)*out_size, out_size, is_batchnorm=True)
        if is_deconv:
            self.up = nn.ConvTranspose2d(in_size+4, out_size, kernel_size=4, stride=2, padding=1)
        else:
            self.up = nn.UpsamplingBilinear2d(scale_factor=2)
