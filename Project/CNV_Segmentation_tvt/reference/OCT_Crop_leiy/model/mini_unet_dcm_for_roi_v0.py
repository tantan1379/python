...
class Mini_Unet_Dcm_For_Roi_V0(nn.Module):
    def __init__(self, in_channels=3, n_classes=3, feature_scale=2, is_deconv=False, is_batchnorm=True):
        super(Mini_Unet_Dcm_For_Roi_V0, self).__init__()
...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm, n=2, ks=3, stride=1, padding=1):
        ...
        # 初始化参数
        for m in self.children(): # self.children 代表的是nn.Sequential层
            init_weights(m, init_type='kaiming')
...
if __name__ == '__main__':
    model = Mini_Unet_Dcm_For_Roi_V0()
    torchsummary.summary(model, (3, 512, 448),device='cpu')