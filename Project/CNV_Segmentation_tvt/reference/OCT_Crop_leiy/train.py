...
from model.mini_unet_dcm_v3 import Mini_Unet_Dcm_V3
...
def eval(model, dataloader, args,k_fold):
    ...
    with torch.no_grad():
        ...
        image_save_name_list = [x.replace('png', 'jpg') for x in image_save_name_list]
        predict_save_name_list = [x.replace('png', 'jpg') for x in predict_save_name_list]
...
def main(mode='train', args=None, writer=None, k_fold=1, device = None, local_rank = None):
    ...
    model_all = {
                 ...
                 'Mini_Unet_Dcm_V3': Mini_Unet_Dcm_V3(in_channels=args.input_channel, n_classes=args.num_classes),

                 # 'Baseline_V0': Baseline_V0(in_channels=args.input_channel, n_classes=args.num_classes),
                }

    model = model_all[args.net_work]
    torchsummary.summary(model, (3, 512, 512),device='cpu') # 输出模型的参数量
