...
class OCT(torch.utils.data.Dataset):
    ...
    PED = [0,0,255]
    Aorta = [255,255,0]
    COLOR_DICT = np.array([Unlabelled,REA,SRF,PED])
    ...
    def read_list(self,image_path):
        ...
        if self.mode=='train':
            ...
            val_list=image_path.replace('trainingset','validset')
            fold_s=sorted(os.listdir(val_list),key=lambda x: int(x[-2:]))

            for item in fold_s:
                img_list+=sorted(glob.glob(os.path.join(val_list,item)+'/*.png'),key=lambda x: (int(x.split('/')[-2][-2:]),int(x.split('/')[-1].split('.')[0])))
            label_list=[x.replace('img','mask') for x in img_list]
            ...
            test_list=image_path.replace('trainingset','validset')
            ...
            label_list=[x.replace('img','test_mask') for x in img_list]
...
#    data = OCT(r'K:\JBHI_OCT\Resize_assign\trainingset', (512, 512),mode='train')
...
#     #             io.imsave(os.path.join(r'G:\KeTi\JBHI_pytorch\Dataset\ffffffffffffff',str(i)+'_'+str(index)+'_'+str(inf)+'.png'),it.astype(np.uint8))


#     #    la=label.data.numpy()
#     #    for index,item in enumerate(la):
#     #        im=data.COLOR_DICT[item.astype(np.uint8)]
#     #        io.imsave(os.path.join(r'G:\KeTi\JBHI_pytorch\Dataset\ffffffffffffff',str(i)+'_'+str(index)+'_'+str(inf)+'_seg'+'.png'),im)
