...
注意new返回的是该数据对应的类型的指针，所以需要用一个指针来接收它。   
...
此时别名称为原名的引用
**Notes:**   
1、引用必须进行初始化: `int &b = a`而不能分成两步：`int &b`+`b = a`   
2、引用一旦初始化，就不能被修改
...
**Notes:**        
1、使用引用传递可以**简化指针** ；     
2、**引用传递** 是在给实参起 **别名**，它没有自己的存储数据的内存位置，它访问的是另一个变量的内存地址。对引用变量作出的任何更改，实际上都是对它所引用的变量内存位置中存储数据的更改；   
3、值传递：形参不会修饰实参；地址传递和引用传递：形参可以修饰实参（含义见下）
...
**值传递和引用传递的区别：**   
值传递（pass-by-value）过程中，被调函数的形式参数作为被调函数的局部变量处理，即在堆栈中开辟了内存空间来存放由主调函数放进来的**实参的值**，从而成为了**实参的一个副本**。值传递的特点是被调函数对形式参数的任何操作都是作为局部变量进行，不会影响主调函数的实参变量的值。   
引用传递（pass-by-reference）过程中，被调函数的形式参数虽然也作为局部变量在堆栈中开辟了内存空间，但这时存放的是**由主调函数放进来的实参变量的地址**。被调函数对形参的任何操作都被处理成**间接寻址**，即通过堆栈中存放的地址访问主调函数中的实参变量。正因为如此，被调函数对形参做任何的操作都影响了主调函数中的实参变量。

##### 地址传递（指针传参）：
指针传参中被调用函数的指针变量在栈内存中重新申请内存
...
3、函数重载遇到默认参数，如果调用一个函数出现二义性，则会报错。
...
**2、值传递的方式给函数参数传值**     
```
void doWork1(Test t){}//值传递，创建一个Test类的副本t，如果在函数内改变t的属性，不会改变test2中的t
void test1() {
	Test t;
	doWork1(t);
}
...
静态多态和动态多态的区别：   
静态动态在编译阶段确定函数地址，动态多态在运行阶段确定函数地址    
...
**Notes:**   
1、C++自动实现父类和子类之间的类型转换   
2、重写指重新写的函数与原函数返回值类型、函数名、参数列表完全相同
3、当发生动态多态时，运行期间自动为父类添加一个指针vfptr[虚函数（表）指针]，该指针指向vftable[虚函数表]，在表的内部记录着虚函数的地址（&Animal::speak)；在子类中会继承这个指针，但由于针对虚函数进行了重写，因此表中的虚函数的地址被覆盖，变为(&Cat::speak)   
4、
