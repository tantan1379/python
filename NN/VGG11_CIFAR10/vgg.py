...
class VGG(nn.Module):
    ...
    def forward(self, x):
        x = self.features(x)
        x = x.view(x.size(0),-1)
        # x = x.view(200, -1)  # 实际上填的是batch_size，由DataLoader确定
        x = self.classifier(x)
        return x
...
def train(epoch, model, writer, train_loader, optimizer, loss_func, device):
    ...
    for batch_num, (data, target) in tqdm(enumerate(train_loader)):
        ...
        optimizer.step()
        # 生成每个数据预测的类别，即概率最大的
        prediction = torch.max(output, 1)  # 返回每行(1)/列(0)的([max],[index])
        total += len(target)
        # 计算准确个数，以用来计算准确率
...
def exam_model(epoch, model, writer, test_loader, loss_func, device):
    ...
    total = 0
    i = 0
    for batch_num, (data, target) in tqdm(enumerate(test_loader)):
        # 测试数据和其对应的标签
        data, target = data.to(device), target.to(device)
        # 将测试数据填入模型中，进行预测类别
        output = model(data)
...
def run(epochs,device):
    ...
    model = VGG().to(device)
    torch.backends.cudnn.benchmark = True  # 花费少量预处理时间，减少卷积计算时间
    # 选择合适的优化器
